    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[60] p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg">
        <form onSubmit={handleSubmit}>
          <div className="p-8">
            <h2 className="text-2xl font-bold text-primary mb-6">{notification ? 'แก้ไขประกาศ' : 'เพิ่มประกาศใหม่'}</h2>
            <div className="space-y-4">
              <input type="text" name="message" value={formData.message} onChange={handleChange} placeholder="ข้อความประกาศ" className={inputClasses} required />
              <div className="grid grid-cols-2 gap-4">
                <input type="text" name="linkLabel" value={formData.linkLabel} onChange={handleChange} placeholder="ข้อความลิงก์ (เช่น ดูเพิ่มเติม)" className={inputClasses} />
                <input type="url" name="link" value={formData.link} onChange={handleChange} placeholder="URL เต็ม (https://...)" className={inputClasses} />
              </div>
              <label className="flex items-center">
                  ...
                  <span className="ml-2 text-gray-700 font-medium">ตั้งเป็นประกาศที่ใช้งาน (จะปิดใช้งานอันอื่น)</span>
              </label>
            </div>
          </div>
