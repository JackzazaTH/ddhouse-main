...
import { PortfolioProject, View, Testimonial } from '@/lib/types';
...
const CardContent: React.FC<{ project: PortfolioProject }> = ({ project }) => (
    ...
    <Image
      ...
      className="object-cover opacity-75 transition-all duration-500 group-hover:opacity-50 group-hover:scale-105"
    />
...
const NextJsPortfolioCard = React.memo(function NextJsPortfolioCard({ project }: { project: PortfolioProject }) {
  return (
    <Link 
...
const SpaPortfolioCard = React.memo(function SpaPortfolioCard({ project, onNavigate }: { project: PortfolioProject; onNavigate: (view: View, slug?: string) => void; }) {
  return (
    <div 
...
const PortfolioPage: React.FC<PortfolioPageProps> = ({ projects, testimonials, onNavigate, onPlayVideo }) => {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="relative bg-gray-800 py-20 sm:py-24 md:py-32 rounded-xl overflow-hidden mb-12">
        <Image src="https://picsum.photos/seed/portfolio-hero/1600/800" fill style={{objectFit:"cover"}} className="opacity-30" alt="Portfolio background" priority />
        <div className="relative text-center mx-auto max-w-2xl px-4">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 animate-fade-in-down">ผลงานของเรา</h1>
          <p className="text-lg text-gray-300 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
            ชมผลงานการสร้างสรรค์บ้านคุณภาพ ที่เราภาคภูมิใจส่งมอบให้กับลูกค้าทุกท่าน
          </p>
        </div>
      </div>
