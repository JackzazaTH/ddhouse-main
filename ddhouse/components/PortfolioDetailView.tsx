  ...
  const BackButton = () => {
    ...
    const content = (
      <>
        ...
        <span>กลับไปที่ผลงานทั้งหมด</span>
      </>
    );
    ...
    <div className="bg-white p-4 sm:p-8 rounded-xl shadow-2xl animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="lg:col-span-3">
          <div className="mb-4">
            <img src={mainImage} alt={project.title} className="w-full h-auto max-h-[500px] object-cover rounded-lg shadow-lg" />
          </div>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="bg-gray-100 p-6 rounded-lg animate-fade-in-up" style={{animationDelay: '200ms'}}>
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">รายละเอียดโครงการ</h2>
            <ul className="space-y-3">
              <li className="flex items-center text-lg">
                <CategoryIcon className="w-6 h-6 mr-3 text-secondary" />
                <strong>ประเภท:</strong><span className="ml-auto">{project.category}</span>
              </li>
              <li className="flex items-center text-lg">
                <LocationIcon className="w-6 h-6 mr-3 text-secondary" />
                <strong>สถานที่:</strong><span className="ml-auto">{project.location}</span>
              </li>
              <li className="flex items-center text-lg">
                <AreaIcon className="w-6 h-6 mr-3 text-secondary" />
                <strong>พื้นที่:</strong><span className="ml-auto">{project.area} ตร.ม.</span>
              </li>
            </ul>
          </div>
