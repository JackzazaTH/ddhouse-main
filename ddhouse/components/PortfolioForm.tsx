...
const PortfolioForm: React.FC<PortfolioFormProps> = ({ project, onSave, onClose }) => {
  ...
  const inputClasses = "p-3 bg-gray-100 border border-gray-300 rounded-md w-full focus:ring-2 focus:ring-accent focus:border-accent outline-none placeholder-gray-500 transition-colors text-gray-800";
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[60] p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <div className="p-8">
            <h2 className="text-2xl font-bold text-primary mb-6">{project ? 'Edit Project' : 'Add New Project'}</h2>
            
            <div className="space-y-4">
                <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="Project Title" className={inputClasses} required />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <input type="text" name="location" value={formData.location} onChange={handleChange} placeholder="Location" className={inputClasses} required />
                 ...
                 <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Cover Image (Main Thumbnail)</label>
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Additional Images (for details page)</label>
