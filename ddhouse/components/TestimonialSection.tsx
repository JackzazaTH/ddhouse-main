...
const TestimonialSection: React.FC<TestimonialSectionProps> = ({ testimonials, onPlayVideo, onViewAll }) => {
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        {featuredTestimonial && (
          <div className="lg:w-2/3">
            <div 
              className="relative rounded-lg overflow-hidden cursor-pointer group shadow-lg aspect-video"
              onClick={() => onPlayVideo(featuredTestimonial.videoUrl)}
            >
              <Image src={featuredTestimonial.imageUrl} alt={featuredTestimonial.title} fill sizes="(max-width: 1023px) 100vw, 67vw" className="object-cover transition-transform duration-300 group-hover:scale-105" />
        ...
        <div className="lg:w-1/3 flex flex-col gap-4">
          {otherTestimonials.slice(0, 3).map(testimonial => (
            <div 
              key={testimonial.id}
              className="relative rounded-lg overflow-hidden cursor-pointer group shadow-md aspect-video"
              onClick={() => onPlayVideo(testimonial.videoUrl)}
            >
              <Image src={testimonial.imageUrl} alt={testimonial.title} fill sizes="(max-width: 1023px) 100vw, 33vw" className="object-cover transition-transform duration-300 group-hover:scale-105" />
