...
import { AreaIcon, BathIcon, BedIcon, ParkingIcon } from './Icon';
    ...
    setFormData(prevData => {
        ...
        if (name === 'discount_percentage') {
            if (discount >= 0 && discount < 100) {
                if (popOrig > 0) updatedData.price_popular_discounted = Math.round(popOrig * (1 - discount / 100));
                if (sigOrig > 0) updatedData.price_signature_discounted = Math.round(sigOrig * (1 - discount / 100));
            }
        } else if (name === 'price_popular_original') {
            if (popOrig > 0 && discount > 0 && discount < 100) updatedData.price_popular_discounted = Math.round(popOrig * (1 - discount / 100));
        } else if (name === 'price_signature_original') {
            if (sigOrig > 0 && discount > 0 && discount < 100) updatedData.price_signature_discounted = Math.round(sigOrig * (1 - discount / 100));
            ...
            if (popOrig > 0 && popDisc >= 0 && popOrig > popDisc) {
                ...
                if (sigOrig > 0) updatedData.price_signature_discounted = Math.round(sigOrig * (1 - newDiscount / 100));
            }
            ...
            if (sigOrig > 0 && sigDisc >= 0 && sigOrig > sigDisc) {
                ...
                updatedData.discount_percentage = newDiscount;
                if (popOrig > 0) updatedData.price_popular_discounted = Math.round(popOrig * (1 - newDiscount / 100));
            }
        }
        return updatedData;
    });
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="relative">
                  <AreaIcon className="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                  <input type="number" name="area" value={formData.area} onChange={handleChange} placeholder="พื้นที่ (ตร.ม.)" className={`${inputClasses} pl-10`} required />
                </div>
                <div className="relative">
                  <BedIcon className="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                  <input type="number" name="bedrooms" value={formData.bedrooms} onChange={handleChange} placeholder="ห้องนอน" className={`${inputClasses} pl-10`} required />
                </div>
                <div className="relative">
                  <BathIcon className="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                  <input type="number" name="bathrooms" value={formData.bathrooms} onChange={handleChange} placeholder="ห้องน้ำ" className={`${inputClasses} pl-10`} required />
                </div>
                 <div className="relative">
                  <ParkingIcon className="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                  <input type="number" name="parking" value={formData.parking || ''} onChange={handleChange} placeholder="ที่จอดรถ" className={`${inputClasses} pl-10`} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                      ...
                      <input type="text" name="style" value={formData.style || ''} onChange={handleChange} placeholder="e.g. Modern, Contemporary" className={inputClasses} />
                  </div>
                  <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Dimensions (W x D)</label>
                      <input type="text" name="dimensions" value={formData.dimensions || ''} onChange={handleChange} placeholder="e.g. 15.5 x 18.0 m." className={inputClasses} />
                  </div>
              </div>
            </div>
            ...
            <div className="mt-6 border-t pt-6">
              ...
              {isPromotionOpen && (
                <div className="mt-4 space-y-4 animate-fade-in">
                    ...
                    <div className={`space-y-4 transition-opacity ${formData.promotion_enabled ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                        <div>
