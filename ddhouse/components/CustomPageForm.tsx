...
const CustomPageForm: React.FC<CustomPageFormProps> = ({ page, onSave, onClose }) => {
  const [formData, setFormData] = useState<Omit<CustomPage, 'id'>>({
    ...
    menuLocation: page?.menuLocation || 'service_submenu',
    order: page?.order || 0,
    seo: {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[60] p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <form onSubmit={handleSubmit} className="flex flex-col h-full min-h-0">
          <div className="p-8 flex-1 overflow-y-auto">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 bg-blue-50 p-4 rounded-lg border border-blue-100">
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Menu Location</label>
                        <select name="menuLocation" value={formData.menuLocation} onChange={handleChange} className={inputClasses}>
                            <option value="service_submenu">Our Services Dropdown (บริการของเรา)</option>
                            <option value="primary">Main Menu (เมนูหลัก)</option>
                            <option value="none">Hidden / Footer Only (ไม่แสดงในเมนู)</option>
                        </select>
                        <p className="text-xs text-gray-500 mt-1">Choose where this page link appears in the header.</p>
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Display Order</label>
                        <input type="number" name="order" value={formData.order} onChange={handleChange} placeholder="0" className={inputClasses} />
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Header/Hero Image</label>
                  ...
                  {formData.imageUrl && (
                      <div className="mt-4">
                          <img src={formData.imageUrl} alt="Header preview" className="w-full h-auto max-h-48 object-cover rounded-md border" />
                      </div>
                  )}
                </div>
            </div>
          ...
          <div className="bg-gray-100 px-8 py-4 flex justify-end space-x-4 border-t flex-shrink-0">
