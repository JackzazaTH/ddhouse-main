    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[60] p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <div className="p-8">
            <h2 className="text-2xl font-bold text-primary mb-6">{page ? 'แก้ไขหน้าเพจ' : 'เพิ่มหน้าเพจใหม่'}</h2>
            
            <div className="space-y-4">
                <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="หัวข้อหน้าเพจ" className={inputClasses} required />
                <input type="text" name="slug" value={formData.slug} onChange={handleChange} placeholder="URL Slug (สร้างอัตโนมัติ)" className={`${inputClasses} bg-gray-200 cursor-not-allowed`} required readOnly />
                <textarea name="content" value={formData.content} onChange={handleChange} placeholder="เนื้อหา (รองรับ HTML)" className={`h-48 ${inputClasses}`} required />
            </div>
