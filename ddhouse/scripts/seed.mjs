...
async function main() {
  ...
  // If not, it means the app is likely being built on Vercel without a connected database.
  // In this case, we skip the seeding process entirely to prevent build failures.
  ...
  // Only if the connection string exists, we dynamically import the database client
  // and proceed with connecting and seeding.
...
main().catch((err) => {
  ...
  // If an error occurs during seeding (when it's supposed to run),
  // we exit with an error code to fail the build.
  process.exit(1);
});
