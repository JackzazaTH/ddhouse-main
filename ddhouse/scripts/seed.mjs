// This script is designed to seed the database, but it includes a critical check
// to prevent build failures on Vercel when a database is not yet connected.
...
async function main() {
  try {
    // Check if the environment variable is available. If not, skip seeding.
    if (!process.env.POSTGRES_URL) {
      console.log("POSTGRES_URL environment variable not found. Skipping database seeding.");
      console.log("This is a normal and expected step if a Vercel Postgres database is not yet connected to the project.");
      return;
    }

    // Dynamically import the database client only when needed.
    ...
    // If ANY error occurs (e.g., DB not ready), log it as a warning but DO NOT fail the process.
    // This ensures that the Vercel build process can continue even if seeding fails.
    console.warn("An error occurred during database seeding, but the build will continue.");
    console.warn("This can happen if the database is not ready during the deployment lifecycle.");
    console.warn("Error details:", error);
  }
}

// Call the main function. We no longer use .catch() to call process.exit(1),
// allowing the script to finish gracefully even if an error is caught internally.
main();
