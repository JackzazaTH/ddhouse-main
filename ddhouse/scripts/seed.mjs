...
// Gracefully skip seeding if the database connection string is not available.
// This prevents Vercel build failures when a database has not yet been connected.
if (!process.env.POSTGRES_URL) {
  console.log("POSTGRES_URL environment variable not found. Skipping database seeding.");
  console.log("This is expected if a Vercel Postgres database is not yet connected.");
...
async function main() {
  ...
  // FIX: Replaced `client.end()` with `client.release()` to correctly release the Vercel Postgres client back to the pool, resolving a type error.
  client.release();
}
